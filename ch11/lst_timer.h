...
public:
    ...
    void add_timer(util_timer* timer) {
        ...
        /* 否则就需要调用重载函数 add_timer(util_timer* time, util_timer* lst_head)，把它插入链表中合适的位置，以保证链表的升序特性 */
    ...
    void del_timer(util_timer* timer) {
        ...
        /* 如果链表中至少有两个定时器，且目标定时器是链表的头节点，则将链表的头节点重置为原头节点的下一个节点，然后删除目标节点 */
        if(timer == head){
            head = head->next;
            head->prev = NULL;
            delete timer;
        ...
        /* 如果链表中至少有两个定时器，且目标定时器是链表的尾节点，则将链表的尾节点重置为原尾节点的前一个节点，然后删除目标节点 */
        if(timer == tail) {
            tail = tail->prev;
            tail->next = NULL;
            delete timer;
        ...
        /* 如果目标定时器位于链表中中间，则把它前后的定时器串联起来，然后删除目标定时器 */
        timer->prev->next = timer->next;
        timer->next->prev = timer->prev;
        delete timer;
    }
...
private:
    ...
    void add_timer(util_timer* timer, util_timer* lst_head) {
        ...
        /* 遍历 lst_head 节点之后的部分链表，直到找到一个超时时间大于目标定时器和超时时间的节点，并将目标定时器插入该节点之前 */
        while(tmp) {
