...
/* 解析 HTTP 请求的一个头部信息 */
http_conn::HTTP_CODE http_conn::parse_header_line(char* text) {
    /* 遇到空行，表示头部字段解析完毕 */
    if(text[0] == '\0') {
        /* 如果 HTTP 请求有消息体，则还需读取 m_content_length 字节的消息体，状态机转移到 CHECK_STATE_CONTENT 状态 */
...
bool http_conn::write() {
    ...
    while(1) {
        ...
        if(temp <= -1) {
            /* 如果 TCP 写缓冲没有空闲空间，则等待下一轮 EPOLLOUT 事件。虽然在此期间，服务器无法立即接收到同一客户的下一个请求，但这可以保证连接的完整性 */
            if(errno == EAGAIN) {
...
bool http_conn::add_linger() {
    return add_response("Connection: %s\r\n", (m_linger == true) ? "keep-alice" : "close");
}
