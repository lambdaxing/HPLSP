...
private:
    ...
    void setup_sig_pipe();
    void run_parent();
    void run_child(const vector<H>& hosts);
...
void processpool<C,H,M>::setup_sig_pipe() {
    ...
    // 往读端关闭的连接中写数据时触发 SIGPIPE 信号，默认会使程序退出
    // SIG_IGN 忽略信号，在 write 的时候返回-1，errno 设置为 SIGPIPE
    addsig(SIGPIPE, SIG_IGN);
}


template< typename C, typename H, typename M >
void processpool<C,H,M>::run(const vector<H>& hosts) {
    if(m_idx != -1) {
        run_child(hosts);
        return;
    }
...
template< typename C, typename H, typename M >
void processpool<C,H,M>::run_child(const vector<H>& hosts) {
    setup_sig_pipe();
    ...
    M* manager = new M(m_epollfd, hosts[m_idx]);
    assert(manager);
